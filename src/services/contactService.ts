...
export class ContactService {
  ...
  static async createContact(data: CreateContactDto): Promise<CreateContactResponse> {
    const res: ApiResponse<CreateContactResponse> = await api.post('/contact', data, {
      requireAuth: false
    });
    return res.data;
  }
  ...
  static async getAllContacts(): Promise<Contact[]> {
    const res: ApiResponse<Contact[]> = await api.get('/contact', {
      requireAuth: true
    });
    return res.data;
  }
  ...
  static async getContactById(id: string): Promise<Contact> {
    const res: ApiResponse<Contact> = await api.get(`/contact/${id}`, {
      requireAuth: true
    });
    return res.data;
  }
  ...
  static async markAsRead(id: string): Promise<Contact> {
    const res: ApiResponse<Contact> = await api.put(`/contact/${id}/read`, {}, {
      requireAuth: true
    });
    return res.data;
  }
  ...
  static async deleteContact(id: string): Promise<DeleteContactResponse> {
    const res: ApiResponse<DeleteContactResponse> = await api.delete(`/contact/${id}`, {
      requireAuth: true
    });
    return res.data;
  }
